...
function handleClick(cell, index) {                     // die Variablen wurden von der Render Funktion übergeben.
  if (fields[index] === null) {                         // wenn dies position = null ist geht er in die 
    ...
    if (isGameFinished()) {
      ...
      document.getElementById('winner').innerHTML = `gewinnt`
    }
  }
}
...
function generateCircleSVG() {
  ...
  document.getElementById('your-choice').innerHTML = ``;
  document.getElementById('your-choice').innerHTML = `${generateCrossSVG()} ist dran`;
...
function generateCrossSVG() {
  ...
  document.getElementById('your-choice').innerHTML = ``;
  document.getElementById('your-choice').innerHTML = `<svg width="${width}" height="${height}">
  <circle cx="35" cy="35" r="30" stroke="${color}" stroke-width="5" fill="none">
    <animate attributeName="stroke-dasharray" from="0 188.5" to="188.5 0" dur="0.2s" fill="freeze" />
  </circle>
</svg> ist dran`;
